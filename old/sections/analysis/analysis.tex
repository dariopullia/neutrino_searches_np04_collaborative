...
\begin{figure}[!hb]
    \centering
    \includegraphics[width=0.78\linewidth]{sections/analysis/TPdistribution/run029424_H4H6_step.png}\\
    \includegraphics[width=0.78\linewidth]{sections/analysis/TPdistribution/run029425_H4H6_step.png}\\
    \includegraphics[width=0.78\linewidth]{sections/analysis/TPdistribution/run032176_H4H6_step.png}
...
Initially, to ensure the robustness of the analysis, only the time periods without a noticeable increase in the TP rate beyond the baseline due to the beamline activity, are considered in these studies. Additionally, time intervals with extra TP activity will be eventually included. %for the sake of triggered-neutrino contributions from beam-induced particles.
...
To identify different shapes of possible showers, we need to analyze their overall shape in the Z direction, along the beam. First and foremost, the space after the vertex Z position is segmented into 10 cm bins. For each bin, the total charge from the collection plane hits is summed for the slice with the highest energy. To eliminate events from slicing failures, we calculate the Region of Interest (ROI) separately from the collection hits. The ROI represents the area in the XZ plane, where we can see the collection plane and the drift, and it exhibits the highest energy deposition. This area should correspond to the interaction that triggered the event.

Initially, two histograms representing the time peaks are generated, assigning weights to each based on the charge detected in the hits. 100 bins are employed and the detector split into two halves at the cathode, concentrating solely on the half exhibiting the highest bin value. Subsequently, we identify the time ROI by adjusting the edges of the peak until the bin value drops below 20$\%$ of the highest value. To calculate the ROI for the Z direction, a histogram of the Z positions is generated. This histogram is then weighted by the charge in the hits, but only for the hits within the specified time ROI. The ROI is determined by adjusting the edges from the peak bin until the bin value falls below 20$\%$ of the maximum (Figure \ref{fig:ROI_scheme}). Finally, the starting and ending points of the ROI are saved for both time and Z directions for further studies.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.59\linewidth]{sections/analysis/ROI_histogram.png}
    \caption{Representation of the ROI finding given a histogram. From the highest peak, the ROI borders are shifted until the counts go lower than 20\% of the maximum bin height.}
...
\end{figure}
To overcome this situation, the idea is to measure the length of the hypothetical \textit{tail} before the vertex, projected onto the Z direction. To do so, the first step is to estimate the region where the tail would be if present, computed by fitting a line through the hits inside the ROI, weighting the hits by charge. The region is selected by adding and subtracting an offset to the fitted line (Figure~\ref{fig:track_region}). 
...
This method can return length values greater than zero even for real neutrinos without any track before the vertex because of pile-up, but there is a clear separation between pile-up and cosmic events with a track physically related to the shower.
...
Table~\ref{tab:filter_evolution} shows the effect of the sequential cuts on the initial dataset, applied from the most to the least impactful. 
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.59\linewidth]{sections/analysis/Selection/sig_directionZ_vs_directionZ2.png}
    \caption{2D histogram showing the $\cos(\theta_{\mathrm{beam}})$ computed with two different reconstruction methods.}
    \label{fig:z1_vs_z2}
\end{figure}
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.49\linewidth]{sections/analysis/Selection/sig_direction_histograms.png}
    \includegraphics[width=0.49\linewidth]{sections/analysis/Selection/bkg_direction_histograms.png}
...
The results of the two functions are not perfectly correlated (Figure~\ref{fig:z1_vs_z2}), indicating that there is room for improvement in the final result by utilizing both methods instead of relying solely on one. 
...
Using the track length variable described in section \ref{chap:Analyzer}, we apply a cut on the ratio between the track length and the distance of the ROI start position from the upstream wall (Figure~\ref{fig:tailDensity}), defined as $$\text{Tail density} = \frac{L_{\text{tail}}}{L_{\text{tot}}} < 0.2~~~~~.$$ 
...
Finally, the ROI along the Z-axis is used to select the events where the neutrino showers develop horizontally, as expected. Consequently, a cut on the ROI is applied, and only events with a sufficiently large ROI size along the Z-axis are selected by requiring $L_{\text{ROI}} > 70$~cm (Figure~\ref{fig:ROIsize}).
